class_name Quests
## Authored story quests â€” the campaign's spine (docs/campaign_starter_system.md).
## Data-driven stages like Research.CHAINS, but with NPC givers, quest text,
## and rewards. Statics persist via SaveGame ("quests").
##
## Stage kinds:
##   contracts â€” complete `n` procedural contracts (MissionLog turn-ins)
##   goto      â€” reach world position `pos` within `radius`; entering the
##               stage charts `poi` (and waypoints it if none is set)
##   report    â€” dock at the station
##   talk      â€” have a conversation with `npc` at `venue` (station|planet):
...
##   scan_target â€” survey a quest-placed Anomaly at `pos` (charts `poi`, the
##               flight scene spawns the Anomaly). Scanning it advances.
##   survive_event â€” fly to `pos` (charts `poi`); arriving triggers a
##               SCRIPTED ambush (flight scene spawns a lesser Cinderweb and
##               suspends the ambient one). Reaching the station's light
##               advances the stage â€” you survive, you don't win.
...
##   goto (+ optional `dialogue`) â€” a goto stage that carries a `dialogue`
##               node dict opens an in-flight DialoguePanel with `npc` on
##               arrival instead of auto-completing; finishing it advances.
##   reach_gate â€” fly into the charted waygate at `pos`: the demo's exit.
...
## complete). One new quest per dock â€” the board hands out work, it doesn't
...
const QUESTS := [
	...
	{"id": "dirtside_run", "title": "Dirtside Run", "layer": "saga", "giver": "ruel",
		"requires": "",
		"body": "The colony is waiting on a crate off the last freighter, and Ruel has nobody dirtside. Fly it down, hand it to Imari â€” and put the hull on the ground without leaving a crater.",
		"briefing": "Ruel finally looks up. \"Before I put you on anything that matters â€” milk run. Colony's short a crate off the last freighter and I've no one dirtside.\" He raps the crate with a knuckle. \"And pilot â€” setting down is NOT docking. There's no lane to line up on. A planet has gravity; it pulls you in whether you're ready or not. Your only job is slowing it. Come in slow, and drop TOWARD the pad, not across it.\" He jerks his chin at your console. \"And use your chart â€” [M] pulls up the nav map. Set the colony as your waypoint and fly the diamond. The Reach is bigger than it looks. Imari signs for the crate.\"",
		...
		"stages": [
			{"kind": "talk", "npc": "imari", "venue": "planet",
				...
				"flash": "Crate delivered. Landing is docking with gravity doing half the work â€” and all of the arguing.",
				"dialogue": {
					"start": {
						"text": "Imari has the crate off your hands before the ramp is fully down. \"From Ruel? Good â€” half this rock's been waiting on seals for that pump.\" She glances past you at your hull, and one corner of her mouth moves. \"And you set her down in one piece. You would be surprised, pilot.\"",
						"choices": [
							{"text": "Any advice for next time?", "next": "advice"},
							{"text": "Glad to help.", "next": "end"},
						]},
					"advice": {
						"text": "\"Same thing every pilot learns the expensive way: a planet is not a dock. Gravity is already pulling you down â€” all you're doing is slowing it. Come in slow and drop STRAIGHT toward the pad. Skim across it and she'll bounce, and bouncing is how a hull becomes a bill.\" She thumps the crate. \"Fly safe.\"",
	...
	{"id": "prove_wings", "title": "Standing With the Board", "layer": "saga", "giver": "ruel",
		...
		"body": "Ruel doesn't hand the interesting work to unknowns. Run contracts off the board â€” bounties, recoveries, deliveries â€” and make your callsign mean something on this dock.",
		"briefing": "Ruel doesn't look up from the ledger. \"New transponder on my dock. Means nothing to me yet.\" He finally meets your eye. \"Board's got work â€” bounties, hauling, salvage recovery. Run a couple, come back in one piece, and maybe your callsign starts meaning something. Off you go.\"",
		"debrief": "\"Twice now you've come back whole. That's twice more than some.\" Ruel actually sets the stylus down. \"Voss â€” the underwriter â€” has been asking after someone exactly this lucky. Go see her before I change my mind about you.\"",
		"stages": [
			{"kind": "contracts", "n": 2,
				"step": "Complete station contracts (%d/%d) â€” any kind counts.",
	...
	{"id": "overdue", "title": "Overdue: The Long Meridian", "layer": "saga", "giver": "voss",
		...
		"debrief": "You tell her: nothing. Not a hull plate, not a fuel sheen. Voss is quiet a long moment. \"No debris.\" She says it flat. \"Pirates leave debris. Storms leave debris. Everything leaves debris.\" The stamp comes down â€” LOST, CAUSE UNKNOWN â€” but her hand doesn't lift off it. \"...Thank you. Take the payment. And keep your comm on.\"",
		"stages": [
			{"kind": "goto", "pos": [6950, 3400], "radius": 420.0, "poi": "meridian_fix",
				...
				"flash": "SWEEP COMPLETE: no debris, no beacon, no fuel sheen. Not wreckage â€” NOTHING."},
			{"kind": "report",
	...
	{"id": "ember_word", "title": "The Word at Ember Row", "layer": "saga", "giver": "voss",
		"requires": "overdue",
		"body": "That empty sweep won't leave Voss alone. She's sent you to Odessa at Ember Row â€” the dock rats hear things no claim file ever records. Pull the thread.",
		...
		"stages": [
			{"kind": "talk", "npc": "odessa", "venue": "station",
				...
				"dialogue": {
					"start": {
						"text": "Odessa pours before you ask. \"You've got the look â€” the one that's stood over an empty patch of space where a ship ought to be.\" She sets the glass down. \"The Meridian wasn't the first. Won't be the last. And not one of them left so much as a scorch on the dark.\"",
						"choices": [
							{"text": "Pirates, surely.", "next": "pirates", "style": "primary"},
							{"text": "What do people say it is?", "next": "stories", "style": "primary"},
						]},
					"pirates": {
						"text": "\"Pirates take the cargo and leave the crew as a message, or take the crew and sell the cargo. They LEAVE things â€” that's the whole point of a message.\" She shakes her head slowly. \"Whatever this is, it doesn't leave. It just... closes over them.\"",
						"choices": [
							{"text": "So what do people say it is?", "next": "stories", "style": "primary"},
							{"text": "That's enough for tonight.", "next": "end"},
						]},
					"stories": {
						"text": "\"Old dock rats call it the thing in the dark. Say it breathes. Say it's got the patience of an orbit â€” comes round when it comes round, and God help what's exposed when it does.\" A dry laugh with nothing behind it. \"I'd call it nerves. If the manifests weren't a body count.\"",
						"choices": [
	...
	{"id": "cold_patch", "title": "The Cold Patch", "layer": "saga", "giver": "lab",
		"requires": "ember_word",
		"body": "The lab wants an instrument reading, not a bar story. There's a residue lingering off the lane where a hauler was lost â€” cold, violet, wrong. Take a survey scanner out and read it directly.",
		"briefing": "The lab tech doesn't look up from the bench. \"Rumors don't calibrate. I need numbers.\" A marker blinks onto your chart. \"There's a residue off the lane â€” right where the Kestrel-class Pale Wager stopped transmitting. Survey scanner, select it, channel a full scan. Bring me what the instruments say, not what the dock rats feel.\"",
		"debrief": "The tech reads your scan twice, then a third time, and the second time the color goes out of their face. \"This isn't a residue. It's an absence. Matter that reads as removed â€” not burned, not scattered. Taken out of the equation.\" They set the slate down very carefully. \"I'm escalating this. Keep your scanner fitted.\"",
		"stages": [
	...
	{"id": "caught_looking", "title": "Caught in the Open", "layer": "saga", "giver": "lab",
		"requires": "cold_patch",
		"body": "The absence had a heading. The lab ran it back through the colony almanac and went quiet. Something passes through a set of coordinates soon â€” and they need a live sensor return the instant it does. You're the rig they're sending.",
		"briefing": "The tech won't quite meet your eye. \"The absence had a vector. I ran it against the old colony almanac and I need you to not ask me what I found.\" A marker burns onto your chart, far out past the lane. \"Something comes through there. Soon. Hold position and record everything. Then come straight home â€” I mean straight â€” the moment your instruments so much as flinch.\"",
		"debrief": "You're still shaking when you reach the bench. The tech takes one look at the return and stops breathing for a moment. \"...No one brings me this. No one brings me anything. That's the whole horror of it â€” they don't come back to bring it.\" They stand, unsteady. \"You did. You're the first. I'm taking this to Ruel and Voss both, tonight. Stay near the light, pilot. Stay near the light.\"",
		"stages": [
			{"kind": "survive_event", "pos": [2800, -2800], "poi": "ambush_site",
				"step": "Hold position at the plotted coordinates â€” then run for the station the instant anything arrives.",
	...
	{"id": "first_contact", "title": "First Contact", "layer": "saga", "giver": "lab",
		...
		"body": "After the last pass, something LINGERED â€” a piece of the web itself, sheared off and drifting where the ambush happened. It has never left a piece of itself before. The lab needs it scanned before it dissipates. A Guardian wing flies escort this time; the Board is done losing pilots to the approach.",
		"briefing": "The tech can barely sit still. \"It left something. It has NEVER left something. A fragment â€” a tendril, sheared off and just... hanging there in the dark.\" A breath. \"Scan it before it's gone. And Ruel's sending a Guardian wing with you â€” real hardware, not volunteers this time. Let them handle whatever's on the lane. You get the reading.\"",
		"debrief": "The scan resolves and the whole bench goes quiet. It isn't residue this time â€” it's TISSUE. Living shadow, structured, purposeful. \"It's not a phenomenon,\" the tech says slowly. \"It's an animal. Old, and patient, and it came from SOMEWHERE.\" They look up. \"Every story says the same word â€” a gate. Go to the colony. Find the one they call the Counter. He's been listening longer than any of us.\"",
		"stages": [
			{"kind": "scan_target", "pos": [3400, -3300], "poi": "tendril_site",
				...
				"flash": "SCAN LOCKED â€” you have a piece of the thing itself. Get it to the lab."},
		],
		"rewards": {"credits": 400, "xp": 80}},
	{"id": "the_hermit", "title": "The Lantern-Drinker", "layer": "saga", "giver": "lab",
		"requires": "first_contact",
		"body": "The colony calls him the Counter â€” an old spacer who has tallied the thing's breathing for thirty years. Land on the planetoid and hear what he knows.",
		"stages": [
			{"kind": "talk", "npc": "hermit", "venue": "planet",
				"step": "Land on the planetoid and find the Counter at the colony's edge.",
				"flash": "The Counter's tally matches the almanac to the day. The thing keeps a schedule â€” and it came through the rim.",
				"dialogue": {
					"start": {
					...
					"what": {
						"text": "\"The old tale calls it the lantern-drinker â€” the sky-web that swallows lights and won't come near a hearth that burns bright enough. That's why your station's still standing. Its light burns bright.\" He finally looks at you. \"For now. The tally says the orbit's tightening.\"",
						"choices": [
							{"text": "And the gate?", "next": "gate"},
						]},
					"gate": {
						"text": "\"It came from somewhere, and it'll go back if we let it â€” or drag us with it. There's an old ring out at the system's rim. Dead since before the colony. The stories say THAT'S the door it came through.\" He turns back to his wall. \"You want to know how to open a dead gate, go ask the only man who's flown the rim and lived. There's a pirate hiding in the Rust Shoal. Krayt. Tell him the Counter sent you, and that the light won't hold.\"",
						"choices": [
	...
	{"id": "rust_shoal", "title": "Ask the Only One Who Ran", "layer": "saga", "giver": "krayt",
		"requires": "the_hermit",
		"grants_shoal": true,   # the hermit hands you Krayt's invitation â€” the Shoal opens to you
		"body": "Krayt â€” a pirate quartermaster gone to ground in the Rust Shoal â€” is the only soul who has flown the system's rim and come home. Fly to the Shoal and get him to talk.",
		"stages": [
			{"kind": "goto", "pos": [2600, -7600], "radius": 520.0, "poi": "rust_shoal", "npc": "krayt",
				"truce": true,       # Krayt grants safe passage â€” pirates hold fire on the player
				"in_person": true,   # you DOCK the Shoal and meet Krayt at his table (not a comm-hail)
				"step": "Fly to the Rust Shoal (charted) and DOCK â€” Krayt will see you in person.",
				...
				"dialogue": {
					...
					"evidence": {
						"text": "\"It is. Not the way WE eat 'em â€” we leave a mess, a ransom, a name somebody curses. This leaves a bill of lading and a hole where a ship used to be.\" The humor drains out of him. \"Six crews. No wreckage, no bodies, no quiet dignity of a grave. Just... gone. I'm the one who ran. I'm also the one still breathing. Do the arithmetic.\"",
						"choices": [
							{"text": "Then help me end it. The gate.", "next": "gate"},
						]},
					"gate": {
						"text": "\"The ring at the rim. Dead metal older than the colony, and it HUMS when the dark passes near it. I've been close enough to feel my teeth ring. I know how to wake it â€” sequence, coordinates, all of it, up here.\" He taps his temple. \"But it's my ticket out too. So here's the deal, hero: you fly cover, I follow you to the rim, and once I'm clear the key is yours. Fair?\"",
						"choices": [
							{"text": "Fair. Get your ship ready.", "next": "deal", "style": "primary"},
						]},
					"deal": {
						"text": "\"Ha. A handshake with a lawman. Never in my life.\" He heaves himself up, almost cheerful now. \"Give me a few to load what's worth loading. Launch, hold off my dock, and I'll form up on you. We run for the rim together â€” first good idea I've had in a year.\"",
						"choices": [
							{"text": "I'll be waiting outside.", "next": "end"},
						]},
				}},
		],
		# KRAYT'S RUNG OF THE SHOAL LADDER (user, 2026-07-27): they OPEN at -100, at war â€”
		# the sanctuary keeps a new pilot safe now, so the Shoal no longer has to start
		# passive. Sitting at his table is what lifts it: -50 is above Standing.HOSTILE_AT,
		# so their guns come off you, but it is still well short of INVITE_AT â€” a truce,
		# not a welcome, and no quests. Vyper's banner later sets 0; breaking it returns
		# you to -100 in one stroke.
		#
		# The stage's `truce` flag holds fire for the APPROACH only, and dies with the beat.
		# Standing is what carries the peace afterwards, which is the whole reason this is a
		# ledger entry and not another boolean.
		"rewards": {"credits": 300, "xp": 80, "standing_floor": {"privateer": -50}}},
	{"id": "nothing_left_behind", "title": "Nothing Left Behind", "layer": "saga", "giver": "krayt",
		"requires": "rust_shoal",
		"manual_start": true,   # NOT auto-handed at a dock â€” the flight set-piece (the Shoal's fall,
		                        # Krayt's final transmission) starts it via Quests.begin_manual().
		"body": "The dark took the Rust Shoal with Krayt still aboard â€” drawn off into the black so the rest could run. His final transmission carried the gate's waking sequence, bought with his life. The ring at the rim is charted. Fly to it, and leave this system before it becomes another empty patch of space.",
		"briefing": "His voice comes stripped of every joke, breathing hard over a sound like the sky closing its mouth: \"...it's got the Shoal, it's got all of it â€” listen. LISTEN. The gate. Coordinates are in the packet. The sequence is three long, two short, hold on the third â€” the ring remembers. Wake it and GO. Don't let this be forâ€”\" Then nothing. But the packet came through. The gate is on your chart.",
		"stages": [
	...
	# ================================================================================
	# THE CINDER REACH CAMPAIGN â€” "The Legend" (docs/cinder_reach_campaign.md)
	# ================================================================================
	...
	#
	# GATED ON `ember_word` â€” the Saga beat where you first walk into Ember Row and meet
	...
	# prereq â€” so the lead is handed where you already are, and the DESTINATION (the
	...
	# Cinderweb, and this Campaign line ends with him GONE (legend_empty_cave â€”
	...
	{"id": "legend_check_in", "title": "Look In On Him", "giver": "odessa", "layer": "campaign",
		...
		# may shake something loose â€” the hunters, maybe the word Ooshu. It is NOT
		...
		"stages": [
			{"kind": "talk", "npc": "odessa", "venue": "station",
				...
				"dialogue": {
					...
					"who": {
						"text": "The rag stops. When she looks up her face has closed like a pressure hatch â€” and it is a practised motion, a thing she has done before, in a hurry.\n\n\"He's an old man who likes his quiet.\" She sets each word down separately, the way you would count out coins. \"That's who he is.\"\n\nShe goes back to the glass. \"Take the bottle or don't.\"",
						"choices": [
							{"text": "Alright. I'll go.", "next": "accept", "style": "primary"},
						]},
					"worried": {
						"text": "She doesn't deny it. From Odessa that is a signed confession.\n\n\"Two nights back I had customers order top-shelf and not drink it.\" The rag goes around the same glass, the same way, three times. \"Sat where they could watch the door. Asked after the colony â€” not the freight, not the beast. After PEOPLE.\"\n\n\"Paid hard currency. Nobody pays hard currency.\"",
						"choices": [
							{"text": "Asking after HIM?", "next": "letslip"},
							{"text": "Report it. The Board, the Guardians â€” somebody.", "next": "board"},
			...
			{"kind": "talk", "npc": "hermit", "venue": "planet",
				...
				"dialogue": {
					...
					"bottle": {
						"text": "He turns â€” and he turns the way a hatch opens on a hinge somebody keeps oiled: square, weight on the back foot, hands empty and visible and READY. His eyes go past you to the cave mouth first. To the light. To how much of it you are standing in.\n\nThen he looks at the bottle, and for a moment he is only an old man.\n\n\"She remembers the label.\" He sets it down unopened. \"She shouldn't have sent anybody. Tell her that. Kindly.\"",
						"choices": [
					...
					"worried": {
						"text": "\"She's right to be.\" He says it the way you would read a gauge â€” no weight on it at all. Then he catches himself, and something kind comes into his voice, and somehow that is worse.\n\n\"Tell her I'm well. Tell her the rock is dry and the counting keeps.\"\n\nAt the mouth of the cave, with the light full on him, he says the last of it not quite to you.\n\n\"If anyone else comes asking after an old man out here â€” anyone at all â€” you'd be doing me a kindness not to remember the way.\"",
						"choices": [
	...
	# BEAT 2 â€” THE EMPTY CAVE. You come back and he is gone, the place is turned over,
	# the smoke has not settled, and scrit are sifting the wreck. What you loot off them
	# is the whole beat: an Ooshu scout drone they smashed for the metal â€” which means
	...
	# as a worry only if three days have actually gone by â€” handed over the moment
	...
	{"id": "legend_empty_cave", "title": "Nobody Home", "giver": "odessa", "layer": "campaign",
		...
		"stages": [
			...
			{"kind": "talk", "npc": "odessa", "venue": "station",
				...
				"flash": "Ooshu. Finders. They don't lose â€” and they were watching his door.",
				"dialogue": {
					"start": {
	...
	# THE LEGEND, beat 3 â€” "Who Is Asking" (user, 2026-07-27).
	#
	...
	# answers â€” every stage trades one closed question for two open ones.
	#
	...
	# player a courier in their own campaign. Nobody sends you to Dex â€” you still have
	...
	# a timer elapsed.
	#
	# GIVER IS `lab`, NOT `odessa` â€” and that is load-bearing twice. The breadcrumb
	...
	#
	# NO `requires_days`. Beat 2 ends on "they were watching his door" â€” a cold gate
	...
	{"id": "legend_who_is_asking", "title": "Who Is Asking", "giver": "lab", "layer": "campaign",
		...
		"stages": [
			{"kind": "talk", "npc": "lab", "venue": "station",
				...
				"flash": "It was a RELAY, not a recorder â€” and it had been out there six weeks. Dex has a bearing.",
				"dialogue": {
					"start": {
						"text": "Dex turns the stalk under the lamp with the delight of a man handed a puzzle and no deadline.\n\n\"Oh, this is lovely. Look â€” no storage. None. It wasn't remembering anything, it was TALKING.\" He taps the shattered lens. \"Eye here, mouth in the housing. Somebody parked it, and somebody else listened.\"\n\nHe frowns at the casing, and some of the delight goes out of him.\n\n\"The corrosion layer, though. That's six weeks of Epharon dust, minimum.\"",
						"choices": [
			...
			{"kind": "talk", "npc": "odessa", "venue": "station",
				"step": "Tell Odessa what the relay said â€” and that its traffic points home.",
...
## Is this quest's prerequisite satisfied â€” including any enforced WAIT?
##
...
## content â€” legend_empty_cave's own briefing says "Nobody's seen him. Three days."
##
...
## answer is to let the story continue â€” a campaign that silently stops is far worse
...
static func _prereq_met(q: Dictionary, req: String, tutorial_done: bool) -> bool:
	...
	if not completed.has(req):
		return false
	# LEVEL GATE â€” the Campaign's cold stretches (user, 2026-07-26). A trail that
...
## WHY the next beat of a chain has not arrived, in the player's words â€” or "" if
...
static func pending_reason(id: String) -> String:
	...
	if req != "" and not completed.has(req):
		return ""   # not cold â€” you simply have not got there yet
...
static func on_dock(is_station: bool, _ship, tutorial_done: bool) -> void:
	# Talk stages resolve at their venue â€” station OR planet (the hermit is
	...
	if is_station:
		for id in active.keys():
			if stage_def(id).get("kind", "") == "report":
				_advance(id)
	# New work is offered at BOTH docks now â€” filtered by whether the giver is
...
## COMPLETES a quest â€” the next giver greets you on the same dock, no re-docking.
...
static func check_new_work(is_station: bool, tutorial_done: bool) -> void:
	for q in QUESTS:
		...
		pending_notes.append("NEW WORK â€” %s: \"%s\"  (Quest Log: Missions tab, or [L] in flight)" % [
...
## stage names an `event` string and whichever ground system performs it calls this â€”
...
## whether to dress a scene for a beat (the wrecked cave) â€” so the world only carries
...
## Beat 7: the waygate â€” the demo's exit. Entering it ends the run.
...
static func _advance(id: String) -> void:
	...
	if active[id].stage >= (q.stages as Array).size():
		...
		for key in (r.get("standing_floor", {}) as Dictionary):
			_raise_standing(str(key), int(r.standing_floor[key]))
		var done_note := "QUEST COMPLETE: %s â€” %s" % [q.title, rewards_text(q)]
		...
		if q.has("debrief"):
			pending_talks.append({"giver": q.giver, "text": q.debrief,
				"quest": "%s â€” COMPLETE" % q.title, "rewards": rewards_text(q),
				"vo": str(id) + "_debrief"})
	else:
		_enter_stage(id)
...
## A BEAT THAT CHANGES A RELATIONSHIP, declared in the quest table rather than hooked up
## in whichever screen happened to present it (user's Shoal ladder, 2026-07-27).
##
## A FLOOR, NOT A SET. "Meeting Krayt signals a truce" is a promise that they will not be
## BELOW -50 afterwards; it is not an instruction to take standing away from a pilot who
## somehow arrived higher. A set would make attending your own truce a punishment in that
## case, and there is no reading of the fiction where the beat should ever cost you.
##
## THROUGH Standing.add, never `points[...] =`. add() carries the guard that flips a
## faction back to PEACE the moment it climbs out of hostile â€” writing the ledger directly
## would leave the Shoal at -50 and still formally at war, which is exactly the half-applied
## state this beat exists to end.
static func _raise_standing(faction: String, floor_at: int) -> void:
	var gap := floor_at - Standing.get_points(faction)
	if gap <= 0:
		return
	Standing.add(faction, gap)
	# VISIBLE, in the name the player knows â€” the ledger key is "privateer", the people
	# who just stopped shooting are the Rust Shoal.
	var note := "%s HOLDS ITS FIRE â€” the truce stands." % \
		Factions.display_name(Factions.faction_for_standing(faction)).to_upper()
	pending_notes.append(note)
	Research.journal.append({"day": GameClock.now(), "text": note})
...
## the instant it isn't (user rule, 2026-07-23 â€” a quest marker must never outlive
...
static func _enter_stage(id: String) -> void:
	...
	# diamond â€” talk stages used to chart nothing and vanish behind side content.
	...
	if poi != "" and PoiMap.exists(poi):
		...
		# so a lingering diamond from the previous beat stuck and a new objective â€”
		# "Ask the Only One Who Ran" at the Rust Shoal â€” never got its own mark until
	...
	for id in active.keys():
		...
		if poi == "":
			...
			if venue == "":
				...
				# them â€” Voss at the station). Without this "The Long Meridian"'s report
...
## THE BEAT THAT IS WAITING, when nothing is active â€” same shape as current_step()
...
## for most of the game â€” the exact failure the banner exists to prevent, arriving
...
## cold â€” you simply have not got there yet â€” and pending_reason() already returns
## "" for that.
...
## PLAYER-FACING SPINE NAMES. Every spine is "The <Something>" (user) â€” The Rise,
## The Legend â€” and each Saga MOVEMENT is too: The Gate, and whatever follows.
##
...
## at either â€” triumphant when you are a scrappy pilot in a junk hull, and much
...
## against TU ADÃ‰N ("the idiots"), the Wardens' name for us â€” and the name the
## GAME itself carries, Rise of AdÃ©n, which states the ending on the title screen
...
## Galeans, not the threat â€” it gives nothing away and gains meaning in hindsight.
...
## Where you are WITHIN the Saga. The spine's title stays put â€” a log entry that
## renames itself underneath the player is disorienting, and the point of listing a
## dormant spine is "this story continues" â€” so the movement rides as position.
##
## SIX MOVEMENTS, EVERY ONE "THE <SOMETHING>" (user, 2026-07-26 â€” recovered from
...
## quest does not exist yet, so the movement stays current once reached â€” which is
## correct: it IS where the story is until something ends it.
const SAGA_MOVEMENTS := [
	{"name": "The Gate", "until": "nothing_left_behind",
		"blurb": "System 1. The crime â€” Cinderweb, the pirates, opening the WayGate."},
	...
	# (TU ADÃ‰N) and the whole late reveal in two words â€” which is why it is
...
## waiting. A debrief is queued the moment a quest completes â€” which can happen
## in flight or at the far venue â€” so filtering at queue time isn't enough. The
## giver waits at their own desk until you get there.
## Talks waiting for ONE person, without draining them â€” for venues that aren't
