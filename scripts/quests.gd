...
const QUESTS := [
	...
	{"id": "rust_shoal", "title": "Ask the Only One Who Ran", "layer": "saga", "giver": "krayt",
		...
		# KRAYT'S RUNG OF THE SHOAL LADDER (user, 2026-07-27): they OPEN at -100, at war —
		...
		# so their guns come off you, but it is still well short of INVITE_AT — a truce,
...
## faction back to PEACE the moment it climbs out of hostile — writing the ledger directly
...
static func _raise_standing(faction: String, floor_at: int) -> void:
	...
	# VISIBLE, in the name the player knows — the ledger key is "privateer", the people
	# who just stopped shooting are the Rust Shoal.
	var note := "%s HOLDS ITS FIRE — the truce stands." % \
