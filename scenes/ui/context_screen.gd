...
## ONE CONTEXT: a list, a detail, an action (docs/person_as_context.md).
##
## Extracted from the two screens built to that design — the Mission Computer and
## Dex's Research Lab — once they existed and could be compared, rather than guessed
## at first and fitted to afterwards. Everything here is what those two turned out to
## have IDENTICALLY; what fills the list and what the detail says stays theirs.
##
##   header = YOU        who you are and what you have, on the right
##   left   = ONE list   sections, rows, and the selection you were reading
##   right  = the selected thing, with its action ON it
##
## THE ACTION HELPER IS THE POINT. `action(text, blocked, on_press)` takes the reason
## it cannot be taken as an ARGUMENT, so greying a button and saying why are the same
## call — a screen cannot do one without the other. Both screens shipped a version of
## that bug within a day of each other (a dead button with nothing beside it), and
## both times the assertion that should have caught it was satisfied by a nearby stat
## line instead. Making it one call is worth more than remembering twice.
##
## Subclasses override `header_text`, `fill_list` and `render_detail`, and use the
## row/widget helpers below. The base owns the shape; the screen owns the words.
...
## Add the rows, using section() / empty_row() / row(). The current `_mode` (if the
## screen declared any) says which list to build.
func fill_list() -> void:
	pass


## Draw `md` — the selected row's metadata — into the detail panel with the widgets
...
## forbidden, so a screen with more than one thing to list switches between them.
...
## Keep the player on the row they were reading; failing that, open on the first real
## row, so the detail panel is never blank while there IS something to read.
...
## both screens shipped one before this helper existed.
...
## by a test (both of this screen's reason assertions passed with the line deleted,
